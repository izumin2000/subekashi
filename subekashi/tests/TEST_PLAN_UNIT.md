...
`key_count`はpt合計をそのまま鍵盤本数として扱う（#1099で`POINTS_PER_KEY`による2pt=鍵盤1本の換算を廃止。従来は`stat-item`の表示・鍵盤ビジュアルの両方が`key_count`(pt合計を2で割った値)を使っており、pt合計が奇数だと表示上端数が切り捨てられていた）。`points`（pt合計そのもの）と`key_count`は現在は常に同値になるが、鍵盤ビジュアル用途（`build_keyboard_geometry`）と画面表示用途（`components/stats_summary.html`・`author.html`）とで意味が異なるため、フィールドとしては引き続き分けている。`tests/test_views.py`の`test_kenreki_stat_value_displays_points_not_song_count_stat`（`StatsViewTest`/`AuthorStatsViewTest`）・`test_stats_summary_shows_kenreki`（`AuthorViewTest`）で回帰防止。
...
| compute_song_pointsと一致 | `compute_kenreki` | view=20、like=2 | `points=4`, `key_count=4` |
| 1曲だけではMAX_KEYSに届かない | `compute_kenreki` | view・likeとも全閾値到達（43pt） | `key_count=43`（< MAX_KEYS）、`overflow_color=None` |
| 空リスト | `compute_kenreki_for_songs` | `[]` | `points=0`, `key_count=0` |
| 1曲のみ | `compute_kenreki_for_songs` | `[(20, 2)]` | `compute_kenreki(20, 2)`と同じ結果 |
| 複数曲のpt合計 | `compute_kenreki_for_songs` | 各曲view=1(1pt)を3曲 | `points=3`, `key_count=3` |
| 曲数が多いほど有利（回帰） | `compute_kenreki_for_songs` | view=1の曲10本 vs view=10の曲1本 | 前者の方が合計pt(10pt)が後者(1pt)より高い（同じ合計viewでも曲を分けた方が有利） |
| MAX_KEYS未満（複数曲） | `compute_kenreki_for_songs` | 全閾値到達の曲を2曲（86pt） | `key_count=86`、`overflow_color=None` |
| MAX_KEYS以上で色分岐開始（複数曲） | `compute_kenreki_for_songs` | 全閾値到達の曲を3曲（129pt） | `key_count=129`（≥MAX_KEYS）、`overflow_color`あり、`overflow_upper_bound=MAX_POSSIBLE_KEY_COUNT` |
