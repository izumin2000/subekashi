...
#### 15-2. `templatetags/number_format.py` — `intcomma(value)`（#334）

**テストファイル**: `tests/test_templatetags_number_format.py`

統計ページの値を3桁カンマ区切りで表示するための自作フィルター。

| テストケース | 入力 | 期待結果 |
| --- | --- | --- |
| 4桁以上の数値 | `1234567` | `"1,234,567"` |
| 3桁以下の数値 | `123` | `"123"`（カンマ無し） |
| ゼロ | `0` | `"0"` |
| `None`（コードレビュー指摘対応） | `None` | `""`（例外を投げない） |
| 数値でない値（コードレビュー指摘対応） | `"abc"` | 例外を投げず入力をそのまま返す |
