...
#### 14-6. `stats` コマンド（月次統計(Stats)の集計・保存、#334。事後登録された過去月の曲の自動反映、#1106）
...
`Song.upload_time`はDBへの登録日時ではなくYouTube動画自体の公開日時のため、過去の年月に公開された動画を今日新規登録すると、その曲の`upload_time`は過去月になる。通常実行は当月分（月初のみ前月分も）しか再計算しないため、そのままでは事後登録した曲の実績が月次統計に反映されないまま放置されてしまう（#1106）。これに対応するため、通常実行のたびに全期間を再計算する（#1094で廃止した方式）のではなく、直近`RETROACTIVE_LOOKBACK_DAYS`(7)日以内に登録された曲（`post_time`基準、`Song.upload_time`とは別のDB登録日時フィールド）の`upload_time`の年月だけを追加で再計算対象に加える。既に当月・前月として再計算対象になっている年月は重複して処理しない。`upload_time=None`の曲は年月を特定できないため対象から除外する。
...
| 事後登録された過去月の曲を自動反映（#1106） | `--force`なし、`upload_time`が1月・`post_time`が今月(3月)の曲が存在し、今月=3月15日として実行 | 当月(3月)分に加えて、事後登録された曲のupload_time月(1月)分も自動で再計算対象に入る |
| lookback期間(7日)より前の登録は対象外（#1106） | `upload_time`が1月・`post_time`が8日前の曲が存在し、今月=3月15日として実行 | 3月分のみが作成され、1月分は再計算されない（実行コストを増やさないための境界） |
| lookback期間ちょうど7日前は対象に含む境界値（#1106） | `post_time`がちょうど7日前の曲が存在 | 1月分も再計算される（`post_time__gte`の境界は含む） |
| 事後登録が既に対象の月と重複しても余計な再計算をしない（#1106） | `post_time`が今月、`upload_time`も今月（月初のため前月分も対象）の曲が存在し、今月=3月1日として実行 | 2月分・3月分のみが作成され、重複や余計な月は作成されない |
| `upload_time`未設定の曲は事後登録検知の対象外（#1106） | `post_time`は今月だが`upload_time=None`の曲が存在 | 例外にならず、当月分のみが作成される |
