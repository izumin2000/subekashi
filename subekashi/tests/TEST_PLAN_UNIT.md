...
2. 対応するAuthorがなく、かつ編集不可の行であれば、そのAuthorAlias自体を実際に所有しているauthor（`source.author`、必ず実在する）の一覧画面へのフォールバック
3. 編集可能な行で対応するAuthorがない場合は表示しない（フォールバック先が自分自身になり無意味なため）
4. 遷移先が現在表示中のページ自身の場合は表示しない
